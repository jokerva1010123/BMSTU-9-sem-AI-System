...
def search(a):
    ...
    if len(conSelected):
        nAll += 1
    if not nAll:
        result.value = pd.DataFrame(nameArr, index=range(0, len(nameArr)), columns=["Название"])
        result.visible = True
    ...
    if not len(res):
        alert.visible = True
        return
    result.value = pd.DataFrame(res, index=range(0, len(res)), columns=["Название"])
    result.visible = True
...
markdownFilterDefault = pn.pane.Markdown("# Найти по фильтрации ", width=800, visible=True)
...
result = pn.widgets.Tabulator(visible=False, formatters=bokeh_formatters)
alert = pn.Row(pn.pane.Alert("# Не найдены такие продукты! Попробуйте ещё раз!"))
alert.visible = False
filter = pn.Column(
    pn.Row(markdownFilterDefault),
